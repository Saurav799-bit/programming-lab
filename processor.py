...
class Processor:
    ...
    def decode(self, instruction):
        ...
        # R-type: add, sub, xor, or, and, sll, srl, sra, slt, sltu, mul, mulh, etc.
        if opcode == 0x33:
        ...
        # I-type (loads): lb, lh, lw, lbu, lhu
            ...
            imm = instruction >> 20
            if imm & 0x800:  # Sign extend if bit 11 is set
                imm = imm | 0xFFFFF000
        ...
        # I-type (ALU): addi, xori, ori, andi, slli, srli, srai, slti, sltiu
            ...
            funct3 = (instruction >> 12) & 0x07
            rd = (instruction >> 7) & 0x1F
            # For shift operations, funct7 is encoded in upper bits of immediate
        ...
        # B-type: beq, bne, blt, bge, bltu, bgeu
            ...
            if imm & 0x1000:  # Sign extend if bit 12 is set
                imm = imm | 0xFFFFE000
            imm = self._to_signed_32(imm)
            rs2 = (instruction >> 20) & 0x1F
            rs1 = (instruction >> 15) & 0x1F
        ...
        # J-type: jal
            ...
            if imm & 0x100000:  # Sign extend if bit 20 is set
                imm = imm | 0xFFE00000
            imm = self._to_signed_32(imm)
            rd = (instruction >> 7) & 0x1F
            rs1 = rs2 = funct3 = funct7 = 0
